...
const { getReimburseDetail, upsertSubmittedReimburse } = useReimburseStore();
const detail = reactive<ReimburseForm>(
  structuredClone(getReimburseDetail(String(route.params.id ?? 'new'))),
);
...
const submitDocument = async () => {
  ...
  upsertSubmittedReimburse(detail, amountTotals.value.total);
